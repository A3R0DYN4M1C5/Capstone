...
def _match_vcs_scheme(url):
    ...
    for scheme in VcsSupport.schemes:
        if url.lower().swith(scheme) and url[len(scheme)] in '+:':
            return scheme
    return None
...
def _ensure_html_header(response):
    ...
    if not content_type.lower().swith("text/html"):
...
class PackageFinder(object):
    ...
    def __init__(
        ...
        # Build find_links. If an argument s with ~, it may be
        # a local file relative to a home directory. So try normalizing
        # it and if it exists, use the normalized version.
        # This is deliberately conservative - it might be fine just to
        # blindly normalize anything ing with a ~...
    ...
    def find_requirement(self, req, upgrade):
        ...
        compatible_versions = set(
            req.specifier.filter(
                ...
                # and  using our own, we can drop the cast to str().
...
def _egg_info_matches(egg_info, canonical_name):
    ...
    """
    try:
        version_ = _find_name_version_sep(egg_info, canonical_name) + 1
    except ValueError:
        return None
    version = egg_info[version_:]
    if not version:
        return None
    return version

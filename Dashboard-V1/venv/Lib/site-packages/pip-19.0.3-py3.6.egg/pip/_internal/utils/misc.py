...
def display_path(path):
    ...
    if path.swith(os.getcwd() + os.path.sep):
...
def is_local(path):
    ...
    if not running_under_virtualenv():
        return True
    return normalize_path(path).swith(normalize_path(sys.prefix))
...
def dist_in_usersite(dist):
    ...
    return norm_path.swith(normalize_path(user_site))
...
def dist_in_site_packages(dist):
    ...
    return normalize_path(
        dist_location(dist)
    ).swith(normalize_path(site_packages))
...
def unpack_file(
    ...
    elif (content_type and content_type.swith('text/html') and
